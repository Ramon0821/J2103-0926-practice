...
<head>
    ...
    <script>
        function delTr(contact_id){
            let flag = confirm("确定删除吗？");
            if (flag){
                location.href="delete_contact?id="+contact_id;
            }
        }
    ...
    <table border="1" class="table table-bordered table-hover">
       ...
       <c:forEach items="${contacts}" var="contact">
        <tr>
            ...
            <td>${contact.email}</td>
            <td>
                <a class="btn btn-default btn-sm" href="update_contact_broke?id=${contact.id}">修改</a>
        ...
        <tr>
            <td colspan="8" align="center" class="form-inline">
                <c:forEach begin="1" end="${pageCount}" var="idx">
                    <a class="btn btn-default ${currentPage == idx?'btn-success':''} "
                       href="query_contact_page?currentPage=${idx}&pageSize=${pageSize}">${idx}</a>
                </c:forEach>
